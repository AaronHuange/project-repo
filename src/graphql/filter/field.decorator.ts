...
export interface FilterFieldOptions {
   ...
   * 2. Select * from user u where concat(u.fname, ' ', u.lname) ilike %alex%. Statement concat(u.fname, ' ', u.lname)
   * is the left side of the conditional statement
   * */
  sqlExp?: string;
}
...
export class GraphqlFilterTypeDecoratorMetadata {
  ...
  constructor(private target) {
    const meta: GraphqlFilterTypeDecoratorMetadata = Reflect.getMetadata(
...
export function FilterField(
  typeFn: ReturnTypeFunc | FilterFieldExcludeOptions,
  options: FilterFieldOptions = {},
) {
